...
export default function MenuCard({ product }: MenuCardProps) {
  ...
  // MUDANÇA AQUI: quantityCount
  const count = cartItem?.quantityCount || 0; 
    ...
    <div className="flex flex-col bg-[#f7f7f7] dark:bg-stone-800 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 border border-stone-200 dark:border-stone-700 h-full">
      ...
      <div className="p-5 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-1">
          <h3 className="text-lg font-bold text-stone-800 dark:text-stone-100 leading-tight">{product.name}</h3>
          <span className="text-luna-gold font-bold text-lg whitespace-nowrap ml-2">R$ {product.price.toFixed(2).replace('.', ',')}</span>
        </div>

        {product.quantity && (
          <p className="text-xs font-bold text-stone-500 dark:text-stone-500 mb-2 uppercase tracking-wide">{product.quantity}</p>
        )}

        <p className="text-stone-600 dark:text-stone-400 text-sm mb-5 flex-grow font-medium leading-relaxed">{product.description}</p>
        ...
        <div className="mt-auto">
          {count === 0 ? (
            ...
            <div className="flex items-center justify-between bg-white dark:bg-stone-700 rounded border border-luna-gold dark:border-luna-gold/50 shadow-sm overflow-hidden h-9">
              <button onClick={(e) => { e.stopPropagation(); decreaseQuantity(product.id); }} className="w-12 h-full flex items-center justify-center text-stone-500 dark:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-600 transition text-lg font-medium">-</button>
              <span className="font-bold text-luna-gold text-base min-w-[20px] text-center">{count}</span>
              <button onClick={(e) => { e.stopPropagation(); addToCart(product); }} className="w-12 h-full flex items-center justify-center text-stone-500 dark:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-600 transition text-lg font-medium">+</button>
            </div>
          )}
        </div>
      </div>
